...
async function proxyAndRewrite(
  targetUrl: URL,
  request: Request,
  context: Context,
  url: URL,
  ...
  if (options.forceReferer) {
    proxyRequest.headers.set('referer', options.forceReferer);
  } else {
...
export default async (request: Request, context: Context) => {
  ...
  // /proxyp/ 路径：在通用代理的基础上自动修复静态资源路径
  if (path.startsWith('/proxyp/')) {
    try {
      let targetUrlString = path.substring('/proxyp/'.length);
      ...
      const proxypPrefix = `/proxyp/${targetUrl.origin}`;
      context.log(`Proxying with path rewrite: ${targetUrl.toString()} -> ${proxypPrefix}`);

      return await proxyAndRewrite(targetUrl, request, context, url, proxypPrefix);
    } catch (error) {
      context.log(`Error proxying path-rewrite URL: ${error}`);
      return new Response(`代理请求失败: ${error}`, {
        status: 502,
        headers: {
          'Access-Control-Allow-Origin': '*',
  ...
  if (targetBaseUrl && matchedPrefix) {
    ...
    try {
      return await proxyAndRewrite(targetUrl, request, context, url, matchedPrefix);
    } catch (error) {
